...
def train_source(config, base_network, classifier_gnn, dset_loaders):
    ...
    parameter_list = base_network.module.get_parameters() +\
...
def adapt_target(config, base_network, classifier_gnn, dset_loaders, max_inherit_domain):
    ...
    random_layer = networks.RandomLayer([base_network.module.output_num(), class_num], config['random_dim'], DEVICE)
    ...
    # configure optimizer
    optimizer_config = config['optimizer']
    parameter_list = base_network.module.get_parameters() + adv_net.get_parameters() \
                     + [{'params': classifier_gnn.parameters(), 'lr_mult': 10, 'decay_mult': 2}]
    optimizer = optimizer_config['type'](parameter_list, **(optimizer_config['optim_params']))
    # configure learning rates
    param_lr = []
...
def upgrade_source_domain(config, max_inherit_domain, dsets, dset_loaders, base_network, classifier_gnn, pseudoLabelArray, pseudoLabelCountPerReiterationArray):
    ...
    ## deleting previously selected instances from test_res
    test_res['sample_masks'] = np.setdiff1d(test_res['sample_masks'],pseudoLabelArray[max_inherit_domain])
    pseudoLabelArray[max_inherit_domain] = np.union1d(test_res['sample_masks'],pseudoLabelArray[max_inherit_domain])
    newPseusoLabelThisDomainCount = len(test_res['sample_masks'])
    pseudoLabelCountPerReiterationArray[max_inherit_domain] = np.append(pseudoLabelCountPerReiterationArray[max_inherit_domain],newPseusoLabelThisDomainCount)
    ...
    return pseudoLabelArray,pseudoLabelCountPerReiterationArray
                                        
                                        
def initializePseudoLabelVariables(config, base_network, classifier_gnn, temp_test_loaders):
    """
    ...
    classifier_gnn.eval()
    pseudoLabelArray = {}
    pseudoLabelCountPerReiterationArray = {}
    for dset_name, test_loader in temp_test_loaders.items():
        pseudoLabelArray[dset_name]=[]
        pseudoLabelCountPerReiterationArray[dset_name]=[]
    return pseudoLabelArray,pseudoLabelCountPerReiterationArray
